...
from .stats_summary import CorrSigResult, CCFResult
import numpy as np
import math
...
def permutation_entropy(x: np.ndarray, order: int = 3, delay: int = 1) -> float:
    from .symbols import ordinal_patterns

    pats = ordinal_patterns(x, order, delay=delay, tie_rule="stable")
    K = np.math.factorial(order)
