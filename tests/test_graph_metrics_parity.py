...
def _networkx_triangles(graph, n):
...
class TestTrianglesMatchHandComputation:
    ...
    def test_hand_computed_graph_also_matches_networkx(self, name):
        ...
        np.testing.assert_array_equal(_networkx_triangles(graph, n), expected)
...
class TestTrianglesMatchNetworkX:
    ...
    def test_random_graphs(self, n, p, seed):
        ...
        got = np.asarray(
            ts2net_rs.triangles_per_node(n, _edges_for_rust(graph, n)), dtype=np.int64
        )
        np.testing.assert_array_equal(got, _networkx_triangles(graph, n))

    def test_the_python_wrapper_agrees_too(self):
        graph = nx.gnp_random_graph(20, 0.25, seed=7)
        got = triangles_per_node(20, _edges_for_rust(graph, 20))
        np.testing.assert_array_equal(got, _networkx_triangles(graph, 20))
...
class TestClusteringMatchesNetworkX:
    ...
    def test_average_clustering(self, seed):
        graph = nx.gnp_random_graph(25, 0.25, seed=seed)
        got = ts2net_rs.clustering_avg(25, _edges_for_rust(graph, 25))
        # ts2net averages only over nodes of degree >= 2; networkx counts
        # degree < 2 nodes as 0. Compare on the same subset.
        local = nx.clustering(graph)
        eligible = [u for u in graph.nodes() if graph.degree(u) >= 2]
