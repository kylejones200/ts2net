...
class TestPipelineE2E:
    ...
    def test_pipeline_output_format(self, known_fixture_dataset, tmp_path):
        ...
        # Sort by series_id for comparison (order may vary)
        results_read_sorted = sorted(results_read, key=lambda x: x['series_id'])
        assert results_read_sorted[0]['series_id'] == 'A'
        assert 'n_edges' in results_read_sorted[0]
        assert 'avg_degree' in results_read_sorted[0]
